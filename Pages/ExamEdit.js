...
export default class ExamEdit extends Component {
  ...
  componentDidMount() {
    this.setState({
      Exam_Name: this.props.route.params['Exam_Name'],
    });
    firestore()
      .collection('Sheets')
      .where('Exam_Name', '==', this.props.route.params['Exam_Name'])
      .get()
      .then(async (d) => {
        if (!d.empty) {
          var ExamData = d.docs[0].data();
  ...
  uploadSheets = async () => {
    ...
    if (this.state.imagePath) {
      ...
      this.state.imageurl = await ref.getDownloadURL();
      firestore()
        .collection('Sheets')
